...
using LinearAlgebra, JLD2, OrdinaryDiffEqSDIRK, OrdinaryDiffEqRosenbrock
...
output_ode = load(rootpath .* "/simulations/data/2A-continuous/25h_0005h_ODE_10runs.jld2")
...
output_sde = load(rootpath .* "/simulations/data/2A-continuous/25h_0005h_SDE_10runs.jld2")
...
# Parameters
pₓ = (V = 100., k = 1.,  μ_max = 5.0,  Kₛ = 0.5, Iₛ = 5.0)
pᵧ = (V = 100., k = 1.,  μ_max = 5.0,  Kₛ = 0.5, Iₛ = 5.0, γ = 2.75)
...
cutoff = 3201
obs = Ysde[:y][401:400:cutoff, 1]

tᵧvec = Ysde[:t][collect(401:400:cutoff)]
Δ = tᵧvec[2]-tᵧvec[1]
...
"""
@showprogress for n in 1:400#(Nₜ-1)
